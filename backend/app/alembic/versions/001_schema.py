"""Full schema (single migration, matches app.models + app.models_dbapi)

Revision ID: 001_schema
Revises:
Create Date: 2026-01-29

Generated to match current models. For future changes use:
...
def upgrade() -> None:
    ...
    # Tables (dependency order; api_assignment before version_commit, then add published_version_id)
    op.create_table(
        "api_group",
    ...
    op.create_table(
        ...
        sa.Column(
            "product_type",
            postgresql.ENUM("postgres", "mysql", name="producttypeenum", create_type=False),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "http_method",
            postgresql.ENUM("GET", "POST", "PUT", "DELETE", "PATCH", name="httpmethodenum", create_type=False),
            nullable=False,
        ),
        sa.Column(
            "execute_engine",
            postgresql.ENUM("SQL", "SCRIPT", name="executeengineenum", create_type=False),
            nullable=False,
        ),
        ...
        sa.Column(
            "access_type",
            postgresql.ENUM("public", "private", name="apiaccesstypeenum", create_type=False),
            nullable=False,
        ),
        sa.Column("sort_order", sa.Integer(), nullable=False),
        sa.Column("created_at", sa.DateTime(), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("param_validates", postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("param_validates_snapshot", postgresql.JSONB(astext_type=sa.Text()), nullable=True),
