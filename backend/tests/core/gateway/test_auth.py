...
def _mock_request(authorization: str | None = None, x_api_key: str | None = None) -> Mock:
    m = Mock()
    m.headers = {}
...
def test_verify_gateway_client_bearer(db: Session) -> None:
    ...
    token = create_access_token(subject=c.client_id, expires_delta=timedelta(seconds=3600))
