...
function TemplateDetailPage() {
  ...
  useEffect(() => {
    ...
    for (const mod of modulesData.data) {
      ...
      ReportModuleService.get(mod.id)
        .then((detail) => {
  ...
  // --- Edit mapping state ---
  const [editMapping, setEditMapping] = useState<SheetMappingPublic | null>(
    null,
  )
  ...
  const _setClientsMutation = useMutation({
    mutationFn: () =>
      ReportModuleService.setTemplateClients(moduleId!, tid, selectedClientIds),
    onSuccess: () => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-4">
        {moduleId && (
          <Link to="/report-management/modules/$id" params={{ id: moduleId }}>
              ...
              {template.sheet_mappings && template.sheet_mappings.length > 0 ? (
                <div className="space-y-4">
                  {template.sheet_mappings.map((mapping) => (
                    <div
                      key={mapping.id}
                      className={`border rounded-lg overflow-hidden ${!mapping.is_active ? "opacity-50" : ""}`}
                    >
                      <Table>
                        <TableBody>
                          <TableRow>
                            ...
                            <TableCell
                              rowSpan={3}
                              className="w-[80px] align-top pt-3"
                            >
                          ...
                          <TableRow>
                            <TableHead className="bg-muted/30 align-top">
                              SQL
                            </TableHead>
                            <TableCell colSpan={5}>
                              <pre className="font-mono text-xs whitespace-pre-wrap bg-muted/50 rounded p-2 max-h-[120px] overflow-y-auto">
                                {mapping.sql_content}
                              </pre>
                            </TableCell>
                          </TableRow>
              ...
              <Table>
                <TableBody>
                  ...
                  <TableRow>
                    ...
                    <TableCell>
                      <Select
                        value={newMapping.write_mode || "rows"}
                        onValueChange={(v) =>
                          setNewMapping({
                            ...newMapping,
                  ...
                  <TableRow>
                    ...
                    <TableCell>
                      <Input
                        type="number"
                        min={0}
                        value={newMapping.gap_rows ?? 0}
                        onChange={(e) =>
                          setNewMapping({
                            ...newMapping,
                            gap_rows: Number(e.target.value),
                          })
                        }
                      />
                    </TableCell>
                  ...
                  <TableRow>
                    <TableHead className="bg-muted/30 align-top pt-3">
                      SQL Content *
                    </TableHead>
                    <TableCell colSpan={3}>
                      <Textarea
                  ...
                  <TableRow>
                    ...
                    <TableCell colSpan={3}>
                      <FormatConfigEditor
                        value={newMapping.format_config}
                        onChange={(v) =>
                          setNewMapping({ ...newMapping, format_config: v })
                        }
                      />
                    </TableCell>
                  </TableRow>
              ...
              <Table>
                <TableBody>
                  ...
                  <TableRow>
                    ...
                    <TableCell colSpan={3}>
                      <FormatConfigEditor
                        value={editForm.format_config}
                        onChange={(v) =>
                          setEditForm({ ...editForm, format_config: v })
                        }
                      />
                    </TableCell>
                  </TableRow>
              ...
              {genResult && (
                <div className="mt-4 p-4 rounded-md border bg-muted/30 space-y-2">
                  ...
                  {genResult.status === "success" && (
                    <div className="flex items-center gap-2">
                      <DownloadButton
                        executionId={genResult.execution_id}
                        fallbackFilename={
                          genResult.output_minio_path?.split("/").pop() ||
                          "report.xlsx"
                        }
                      />
                      <Button
...
function CellFormatPills({ label, fmt }: { label: string; fmt: CellFormat }) {
  ...
  if (fmt.font?.color)
    pills.push({ text: `#${fmt.font.color}`, color: fmt.font.color })
  if (fmt.fill?.bg_color)
    pills.push({ text: "fill", bgColor: fmt.fill.bg_color })
  ...
  return (
    <div className="flex items-center gap-1.5 flex-wrap">
      ...
      {pills.map((p, i) => (
        <span
          key={i}
          className="inline-flex items-center gap-1 text-[11px] bg-muted rounded px-1.5 py-0.5"
        >
          {p.color && (
            <span
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        {config.auto_fit && (
          <span className="inline-flex items-center text-[11px] bg-blue-500/15 text-blue-400 rounded px-1.5 py-0.5">
            auto-fit
            {config.auto_fit_max_width ? ` ≤${config.auto_fit_max_width}` : ""}
          </span>
        )}
        {config.wrap_text && (
          <span className="inline-flex items-center text-[11px] bg-blue-500/15 text-blue-400 rounded px-1.5 py-0.5">
          ...
          <Table>
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell>
                  {template.recalc_enabled ? "Enabled" : "Disabled"}
                </TableCell>
              </TableRow>
              <TableRow>
          ...
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell>
                <FileSelect
                  datasourceId={minioDsId}
                  bucket={moduleDetail?.default_template_bucket}
